 ...
 *   .: Descrição...Tela: Formulário de cadastro de peça de garantia para linha de motocicleta.
 */
--%>
...
<html:html>
		...
		<script language="JavaScript">
			...
			function removerArquivo(fileId) {
				...
				if(window.confirm("Confirma e remoção do arquivo?")) {
					__showAnima();
					submitFormTsk(garantiaPecaMotoForm,'removerArquivo&fileId=' + fileId,true);
				}
			...
			function voltarSG() {
				...
				window.location.href = "GarantiaMoto.do?task=alter&readonly=" + <core:out value="${consulta}"/> + "&garantiaId=" + window.document.garantiaPecaMotoForm.entityId.value;
			...
			function validateLine(lineId) {
				...
				var item  = eval( "document.garantiaPecaMotoForm.itemCode_" + lineId );
			...
			function storeItem(field) {
					...
					var itemFieldSource   = eval("window.document.garantiaPecaMotoForm.itemCode_" + field);
					var itemFieldTarget   = eval("window.document.garantiaPecaMotoForm.escapedItem_" + field);			
			...
			function posGetItemCode(){
				...
				var tgItemCode = window.document.garantiaPecaMotoForm.targetItemCode.value;
				...
				if ( tgItemCode != "NOT FOUND" ) {				
					...
					if ( vItemCounter > 1 ) {
						...
						for ( i = 1 ; i < vItemCounter ; i++ ){
							...
							var objItemIdAnt = eval("window.document.garantiaPecaMotoForm.itemId_" + i);
					...
					var objDescItem   = eval("window.document.garantiaPecaMotoForm.descItem_" + vItemCounter);
					var objItemId     = eval("window.document.garantiaPecaMotoForm.itemId_" + vItemCounter);
					var itemCode      = eval( "document.garantiaPecaMotoForm.itemCode_"  + vItemCounter);
				...
				window.document.garantiaPecaMotoForm.targetItemCode.value = "";			
			}
			...
			function addRow() {
				...
				if ( counter > 1 ) {
					...
					var qtdePecaAnt   = eval( "document.garantiaPecaMotoForm.qtdPeca_"  + ant);
					var itemCodeAnt   = eval( "document.garantiaPecaMotoForm.itemCode_" + ant);
					var vItemCodeAnt  = itemCodeAnt.value;
					var faltanteAnt   = eval( "document.garantiaPecaMotoForm.faltante_" + ant);
					var enviarAnt     = eval( "document.garantiaPecaMotoForm.enviar_"   + ant);
					var cobrarAnt     = eval( "document.garantiaPecaMotoForm.cobrar_"   + ant);
					var isEnviar      = true;
					...
					//window.alert("qtde:" + qtdePecaAnt.value + " indexPecaAnt:" + indexPecaAnt + " vIdentPecaAnt:" + vIdentPecaAnt); 
			...
			function addFilledRow(pecaId, lineId, pecaCode, pecaDesc, qtd, enviar, cobrar, faltante, causadora) {
				...
				//window.alert("Adicionando linha " + lineNumber + " - Line Id: " + counter + " - Index: " + index + " - Causadora: " + causadora);
				...
				//window.alert("enviarCheck: " + enviarCheck + " - cobrarCheck: " + cobrarCheck + " - faltanteCheck: " + faltanteCheck + " - causadoraCheck: " + causadoraCheck);
			...
			function checkCausadora(counter) {				
				for (var i=0 ; i < window.document.garantiaPecaMotoForm.elements.length ; i++) {
					var x = window.document.garantiaPecaMotoForm.elements[i];
			...
			function deleteAlterRow(index) {
				...
				//if ( causadora.checked == "checked" ) {
					...
					if ( window.confirm("Confirma exclusão da peça?") ) {
						...
						var linha = eval( "document.garantiaPecaMotoForm.linha_" + index );
						var garantiaId = document.garantiaPecaMotoForm.entityId.value;
					
						window.location.href = "GarantiaPecaMoto.do?task=removeLine&garantiaId=" + garantiaId + "&lineId=" + linha.value + "&isZeroKm=" + <core:out value='${isZeroKm}'/>;
			...
			*/
			...
			function validateForm() {
				...
				var counter = document.forms[0].counter.value;
				var onlyServ = window.document.garantiaPecaMotoForm.onlyService;
				...
				__showAnima();
				window.document.garantiaPecaMotoForm.salvar.disabled = true;
				submitFormTsk(garantiaPecaMotoForm,'save',true);
	...
	<body leftmargin="0" topmargin="3" onLoad="__loadEsconde();">
		...
		<center>
			...
			<html:form action="/GarantiaPecaMoto" method="post" enctype="multipart/form-data">
				...
				<table width="98%" border="0" cellpadding="0" cellspacing="2" align="center">
					...
					<tr height="25"> 
						...
						<td width="11%" class="text">	
							<bean:write name="garantiaPecaMotoForm" property="loteId"/>
						</td>
						<td width="13%" class="td_dark">	
						...
						<td width="22%" class="text">	
							<bean:write name="garantiaPecaMotoForm" property="chassi"/>
						</td>
					</tr>
					<tr height="25"> 
						<td class="td_dark">	
						...
						<td class="text">	
							<bean:write name="garantiaPecaMotoForm" property="numeroOS"/>
						</td>
						<td class="td_dark">	
							<bean:message key="garantia.form.peca.dataAbert"/>:
						</td>
						<td class="text">	
							<bean:write name="garantiaPecaMotoForm" property="dataAberturaOS"/>
						</td>
						<td class="td_dark">	
							<bean:message key="garantia.form.peca.dataFech"/>:
						</td>
						<td class="text">	
							<bean:write name="garantiaPecaMotoForm" property="dataFechamentoOS"/>
						</td>
					</tr>
				</table>
				<p/>
				...
				<table border="0" cellpadding="0" cellspacing="0" width="700">
					<tr>
						<td  width="70%" height="10" class="text" align="left">
							<html:image srcKey="form.moto.img.required" altKey="form.msg.required"/>&nbsp;-&nbsp;<bean:message key="form.msg.required"/>							
						</td>
				...
				<logic:notEmpty name="garantiaPecaMotoForm" property="listFiles">
					...
					<logic:iterate name="garantiaPecaMotoForm" property="listFiles" id="file">
						
						<tr>
							<td class="text" width="80%">
								<html-el:link action="/GarantiaPecaMoto.do?task=download&fileId=${file.entityId}"
							...
							<td class='text' align="center">
								<html-el:link href="#"  onclick="javascript:removerArquivo(${file.entityId})" onmouseover="window.status='${file.filename}';return true;"	onmouseout="window.status='';return true;">
				...
				<table border="0" cellpadding="0" cellspacing="0" width="700">
					<tr>
						<td height="10" class="text" align="left">
							<html:image srcKey="form.moto.img.required" altKey="form.msg.required"/>&nbsp;-&nbsp;<bean:message key="form.msg.required"/>
							<html:image srcKey="form.moto.img.required" altKey="garantia.form.peca.msg.causadora"/>
							<html:image srcKey="form.moto.img.required" altKey="garantia.form.peca.msg.causadora"/>&nbsp;-&nbsp;<bean:message key="garantia.form.peca.msg.causadora"/>							
						</td>		
					</tr>
				</table>
				<logic:notEmpty name="garantiaPecaMotoForm" property="listFiles">
