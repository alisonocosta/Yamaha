 ...
 *   .: Criação.....13 de outubro de 2012, 10:58
 *   .: Autor.......Edson Luiz Sumensari
 *   .: Descrição...Tela: Listagem de solitações de garantias da linha de motocicleta.
 */
--%>
...
<html:html>
	<head>
		...
		<script type="text/javascript">
			...
			function incluir(loteId) { 
				...
				window.location.href = "GarantiaMoto.do?task=add&loteId=" + loteId;
			...
			function confirmRemove(loteId, garantiaId) {
				...
				if ( window.confirm("Confirma a exclusão da Sol. e Garantia Nº " + garantiaId + "?") )
					window.location.href = "GarantiaMoto.do?task=delete&loteId=" + loteId + "&garantiaId=" + garantiaId;
	...
	<div id="carregador_pai">
		...
		<html:form action="/GarantiaMoto" method="get">
			...
			<center>
				...
				<br>
				<display:table name="requestScope.garantiaMotoForm.listGarantias" 
					...
					<display:column titleKey="garantia.list.actions" media="html" headerClass="headerAlign">
						<center>
							<c:if test="${garantia.hasCampaign == false}">
								<html-el:link action="/GarantiaMoto.do?task=alter&garantiaId=${garantia.entityId}&readonly=true" 
							...
							<c:if test="${garantia.hasCampaign == true}">
								<html-el:link action="/GarantiaMoto.do?task=alterCampaign&garantiaId=${garantia.entityId}&readonly=true" 
											  style="text-decoration: underline;"
											  onclick="consulta()"
											  onmouseout="window.status='';return true;"
								   			  onmouseover="window.status='Consultar';return true;">
							...
							<c:if test="${validAlter == true && (garantia.validAlterGarantia == true)}">	
								...
								<c:if test="${garantia.hasCampaign == true}">
								<html-el:link action="/GarantiaMoto.do?task=alterCampaign&garantiaId=${garantia.entityId}" 
											  style="text-decoration: underline;"
											  onclick="alter()"
											  onmouseout="window.status='';return true;"
								...
								<c:if test="${garantia.hasCampaign == false}">
								<html-el:link action="/GarantiaMoto.do?task=alter&garantiaId=${garantia.entityId}" 
											  style="text-decoration: underline;"
											  onclick="alter()"
											  onmouseout="window.status='';return true;"
								   			  onmouseover="window.status='Alterar';return true;">
