...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function save() {
				__showAnima();
				submitFormTsk(window.document.garantiaForm,"saveCampaign",true);
			...
			function cancel( loteId ) {
				...
				__showAnima();
				window.location.href = "Garantia.do?task=cancelIncludeCampaign&loteId=" + loteId;
			...
			*/
			...
			function gravarForm(form) {
	    		...
	    		<core:if test="${idTipo == 1}">
	    			...
	    			var phasKM = true; 
	    			if ( window.document.garantiaForm.hasKM.value == "false" )
	    				phasKM = false;
			...
			function checkLen(x,y) {
				if (y.length==x.maxLength) {
					...
					if ( isThis == "dtAbert" ) {
						...
						if ( window.document.getElementById("garantiaForm").km != undefined )
		...
		<html:form action="/Garantia" method="post" focus="chassi">			
			...
			<tr> 
				...
				<td  width="75%" class="text">
					...
					<display:table name="requestScope.garantiaForm.listServicos" 
			...
			<tr> 
				...
				<td  width="75%" class="text">
					...
					<display:table name="requestScope.garantiaForm.listPieces" 
				               requestURI="${pageContext.request.contextPath}/Garantia.do"
				               export="false" 
				               id="peca" 
				               class="grid">
