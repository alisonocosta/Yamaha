 ...
 */
public class AnaliseGarantiaAction extends InfraDispatchAction {

	private static final long serialVersionUID = -3207678520067532152L;
	...
	/** Tarefa: Apresenta form de pesquisa de garantias para analise.
     * 
     *  @param mapping
     *      O objeto <code>ActionMapping</code> utilizado para
     *      selecionar esta instância.
    ...
    public ActionForward showForm( ActionMapping mapping
    	...
    	try { 
    		...
    		if(!"".equals(innerCode)) {
	            ...
	            List results = analiseGarantiaBusiness.listByParametro(null, ConstantAnaliseGarantia.TIPO_POR_LINHA, user.getSegmento());            

    			if(results != null && !results.isEmpty()) {         
    				gAnaliseForm.set("listResults", results);    			
    			} else {
    				gAnaliseForm.set( "listResults" , new ArrayList(0));
    			}
	            
	            request.setAttribute("listOptionsSearch" , ConstantAnaliseGarantia.listValuesSearch(user.getSegmento()));	 	            	   
    ...
    public ActionForward list( ActionMapping mapping
    	...
    	try {
    		...
    		String parametro = gAnaliseForm.getString("parametro");            
    		String criterio  = gAnaliseForm.getString("criterio");
    		Usuario usuario = (Usuario) UserHelper.getBoundedUser(request.getSession());
    		
    		if(parametro == null || criterio == null || "".equals(parametro) || "".equals("criterio")) {
    			messagesProblem.add( ActionMessages.GLOBAL_MESSAGE, new ActionMessage("analiseGarantia.msg.problem.parametros.pesquisa") );
    			super.addProblemMessages(request, messagesProblem);	            
    		}  else {  

    			AnaliseGarantiaBusiness  analiseGarantiaBusiness  = (AnaliseGarantiaBusiness)  springContext.getBean("analiseGarantiaBO");    			

    			List results = analiseGarantiaBusiness.listByParametro(parametro.trim(), criterio.trim(), usuario.getSegmento());            

    			if(results != null && !results.isEmpty())            
    				gAnaliseForm.set("listResults", results);
    			else {
    				messagesWarning.add( ActionMessages.GLOBAL_MESSAGE, new ActionMessage("analiseGarantia.msg.warning.results") );
    				super.addWarningMessages(request, messagesWarning);
    			}

    		}           

    		request.setAttribute("listOptionsSearch" , ConstantAnaliseGarantia.listValuesSearch(usuario.getSegmento()));	
    ...
    public ActionForward analisar( ActionMapping mapping
    	...
    	if(garantiaIdStr != null && !"".equals(garantiaIdStr)) {
    		...
    		throw new ControllerException("Garantia inválido!");
    ...
    private void preparaForm(DynaActionForm gAnaliseForm, Integer garantiaId) throws ControllerException {
    	...
    	try { 
    		...
    		/* - INICIO - Alteração em 08/09/2014  ist_edson  - Inclusão dos campos Condição sinteom e valor de Serv. de Terceiros */
    		gAnaliseForm.set( "sintoma"     	, garantia.getSintoma() != null ? garantia.getSintoma().getDescricaoCode()  : "" );
    		gAnaliseForm.set( "condicao"    	, garantia.getCondicao()!= null ? garantia.getCondicao().getDescricaoCode() : "" );
    		
    		gAnaliseForm.set( "servTer"   		, NumberUtils.formatNumberCurrency(garantia.getValorServicoTerceiro()));
    		/* - FIM - Alteração em 08/09/2014  ist_edson  - Inclusão dos campos Condição sinteom e valor de Serv. de Terceiros */
