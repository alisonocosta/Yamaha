 ...
 *   .: Objeto......InsertDiagnosticoProblemaAjaxAction.java
 *   .: Criação.....03 de Outubro, 13:08
 *   .: Autor.......Edson Luiz Sumensari
 *   .: Descrição...Action de serviço para inserir um item de Diagnóstico do problema para uma Informações de Mercado.
 */
...
/** Action de serviço para inserir um item de Diagnóstico do problema para uma Informações de Mercado.
 * 
 *  @author Edson Luiz Sumensari
 */
public class InsertDiagnosticoProblemaAjaxAction extends InfraAjaxDefaultAction {
    ...
    public String getXmlContent( ActionMapping mapping
    	...
    	try {
    		...
    		if ( request.getParameter("entityId") != null ) {
	    		...
	    		String diagnosticoTxt   = URLDecoder.decode( request.getParameter("diagnosticoTxt"), "ISO-8859-1" );
	    		...
	    		if ( diagnosticoTxt != null && !"".equals(diagnosticoTxt) ) {
		    		...
		    		if ( infoMercado != null ) {
						...
						histDiagnostico.setInfoMercadoId( new Integer( infoMercado.getEntityId().toString() ) );
						histDiagnostico.setDiagnostico( diagnosticoTxt );
						
						ControllerHelper.prepare( histDiagnostico, (Long)usuario.getIdentifier() );
							
						infoMercadoBusiness.getInfoMercadoDao().makePersistent(histDiagnostico);   			
		    			
						//System.out.println("Recuperando as informações gravadas! ID: " + infoMercado.getEntityId());
						
						String history = infoMercadoBusiness.getHistory( InfoMercadoBusiness.HIST_DIAGNOSTICO, infoMercado.getEntityId() );
						
						//System.out.println("Histórico recuperado: " + history);
						
		    			builder.addItem("diagnostico"   ,  history);
