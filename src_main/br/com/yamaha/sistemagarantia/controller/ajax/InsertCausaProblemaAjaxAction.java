 ...
 *   .: Objeto......InsertCausaProblemaAjaxAction.java
 *   .: Criação.....03 de Outubro, 12:22
 *   .: Autor.......Edson Luiz Sumensari
 *   .: Descrição...Action de serviço para inserir um item de Causa do problema para uma Informações de Mercado.
 */
...
/** Action de serviço para inserir um item de Causa do problema para uma Informações de Mercado.
 * 
 *  @author Edson Luiz Sumensari
 */
public class InsertCausaProblemaAjaxAction extends InfraAjaxDefaultAction {
    ...
    public String getXmlContent( ActionMapping mapping
    	...
    	try {
    		...
    		if ( request.getParameter("entityId") != null ) {
	    		...
	    		String causaTxt   = URLDecoder.decode( request.getParameter("causaTxt"), "ISO-8859-1" );
	    		...
	    		if ( causaTxt != null && !"".equals(causaTxt) ) {
		    		...
		    		if ( infoMercado != null ) {
	    				...
	    				histCausaProblema.setInfoMercadoId( new Integer( infoMercado.getEntityId().toString() ) );
	    				histCausaProblema.setCausaProblema( causaTxt );
						
						ControllerHelper.prepare( histCausaProblema, (Long)usuario.getIdentifier() );
							
						infoMercadoBusiness.getInfoMercadoDao().makePersistent(histCausaProblema);   			
		    			
						//System.out.println("Recuperando as informações gravadas! ID: " + infoMercado.getEntityId());
						
						String history = infoMercadoBusiness.getHistory( InfoMercadoBusiness.HIST_CAUSA, infoMercado.getEntityId() );
						
						//System.out.println("Histórico recuperado: " + history);
						
		    			builder.addItem("causaProblema"   ,  history);
